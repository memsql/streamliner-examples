 ...
 *
 * Modified for MemSQL Streamliner examples
 */
...
object CheckpointedKafkaUtils extends Logging {
  ...
  def createDirectStreamFromZookeeper[K: ClassTag, V: ClassTag, KD <: Decoder[K]: ClassTag, VD <: Decoder[V]: ClassTag] (
                                      ...
                                      topics: Set[String],
                                      batchInterval: Long,
                                      lastCheckpoint: Option[Map[String, Any]]): CheckpointedDirectKafkaInputDStream[K, V, KD, VD, V] = {
    ...
    new CheckpointedDirectKafkaInputDStream[K, V, KD, VD, V](
